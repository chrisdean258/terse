...
pub enum Error {
    BinOp(Span, TypedExpr, BinOpKind, TypedExpr),
}
...
impl TypeChecker {
    ...
    */
    ...
    fn binop(
        ...
        let res = match (&left.type_, op, &right.type_) {
            (a, BinOpKind::Add, b) => a + b,
            (a, BinOpKind::Subtract, b) => a - b,
            (a, BinOpKind::Multiply, b) => a * b,
            (a, BinOpKind::Mod, b) => a % b,
            (a, BinOpKind::Divide, b) => a / b,
            (Type::Integer, BinOpKind::IntegerDivide, Type::Integer) => Some(Type::Integer),
            _ => None,
        };
        let Some(type_) = res else {
            return Err(vec![Error::BinOp(span.clone(), left, op, right)]);
        };
        return Ok(TypedExpr {
            span,
            type_,
            value: TypedExprKind::RValue(TypedRValueKind::BinOp {
                left: Box::new(left),
                op,
                right: Box::new(right),
            }),
